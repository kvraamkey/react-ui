...
const importView = (DemoComponentName) => React.lazy(() => import(`./components/${DemoComponentName}`).catch((e) => console.log("demo component not exported")));
            ...
            <section className='docs-nav animate-opacity'>
                ...
                <div className='docs-nav-list'>
                    {Object.keys(library).map((lib) => (
                        <span className={urlParams.get("c") === lib ? "active" : ""} key={lib} onClick={() => loadViews(lib)}>
                            {lib}
                        </span>
                    ))}
                </div>
            </section>
            <React.Suspense fallback='loading views'>{views}</React.Suspense>
        </section>
    );
};
